...
# --- FRAME-SKIP AUGMENTATION (fork addition) ---------------------------------------------------------------
# Teach the ranker that cells CAN move far in one apparent step: add (t, t+k) training windows whose positives
# are the k-step GT lineage. This manufactures large-displacement positives (gap 2/3 -> 6-8um, the exact range
# of the fast-motion misses) that gap-1 training almost never sees. Default off = stock training.
FRAMESKIP_PROB = float(os.environ.get("BIOHUB_FRAMESKIP_PROB", "0.0"))   # per (t,k) chance to add a skip window
FRAMESKIP_MAXK = int(os.environ.get("BIOHUB_FRAMESKIP_MAXK", "3"))       # max frame gap k
FRAMESKIP_SEED = int(os.environ.get("BIOHUB_FRAMESKIP_SEED", "0"))


def compute_kstep_transition_matrix(
    gt_ids_t: np.ndarray,
    gt_ids_tk: np.ndarray,
    children: dict,
    gap: int,
) -> torch.Tensor:
    """GT transition matrix for a (t, t+gap) skip window: a(t)->b(t+gap) if b is a's descendant `gap` edges down."""
    tk_to_col = {int(nid): j for j, nid in enumerate(gt_ids_tk)}
    matrix = torch.zeros(len(gt_ids_t), len(gt_ids_tk), dtype=torch.float32)
    for i, a in enumerate(gt_ids_t):
        frontier = {int(a)}
        for _ in range(gap):
            frontier = {c for n in frontier for c in children.get(n, ())}
        for b in frontier:
            j = tk_to_col.get(b)
            if j is not None:
                matrix[i, j] = 1.0
...
def get_window_data_skip(
    ...
    """Frame-skip window: frame A at t_start, frame B pulled from t_start+gap but TIME-LABELLED t_start+1
    (so the model sees an *adjacent* transition with a large spatial jump). Positives = gap-step GT lineage."""
    gt_attrs = gt_graph.node_attrs(attr_keys=["node_id", "t", "z", "y", "x"])
    ...
    coords_list: list[torch.Tensor] = []
    node_counts: list[int] = []
    # (source frame the nodes come from, time-label the model sees)
    for src_t, time_label in ((t_start, t_start), (t_start + gap, t_start + 1)):
        gt_t = gt_attrs.filter(pl.col("t") == src_t)
        if len(gt_t) == 0:
            return None
        gt_coords_t = gt_t.select(["z", "y", "x"]).to_numpy().astype(np.float32) / ds
        gt_ids = gt_t["node_id"].to_numpy()
        n_gt = len(gt_coords_t)
        full_coords = np.column_stack([np.full(n_gt, time_label, dtype=np.float32), gt_coords_t])
        pos_feats.append(torch.from_numpy(extract_pos_features(full_coords, image_shape)))
        coords_list.append(torch.from_numpy(gt_coords_t))
        per_frame_ids.append(gt_ids)
        node_counts.append(n_gt)
    target = compute_kstep_transition_matrix(per_frame_ids[0], per_frame_ids[1], children, gap)
    if target.sum() == 0:
        return None  # no lineage survives the gap in these frames -> nothing to learn
    return FrameWindowData(
...
def load_dataset_windows(
    ...
    # frame-skip augmentation: add (t, t+k) windows with k-step lineage positives (large-displacement)
    if FRAMESKIP_PROB > 0.0 and window_size == 2:
        ea = tracks.edge_attrs(attr_keys=["source_id", "target_id"])
        children: dict[int, list[int]] = {}
        for s, t in zip(ea["source_id"], ea["target_id"]):
            children.setdefault(int(s), []).append(int(t))
        rng = np.random.default_rng(FRAMESKIP_SEED + int(image_shape[0]))
        n_skip = 0
        for t in range(image_shape[0]):
            for k in range(2, FRAMESKIP_MAXK + 1):
                if t + k >= image_shape[0] or rng.random() >= FRAMESKIP_PROB:
                    continue
                data = get_window_data_skip(tracks, image_shape, t, k, children, downsample=downsample)
                if data is not None:
                    windows.append(data)
                    n_skip += 1
        print(f"  frame-skip: +{n_skip} skip windows over {len(windows)-n_skip} base "
              f"(prob={FRAMESKIP_PROB}, maxk={FRAMESKIP_MAXK})", flush=True)
...
def train(
    ...
    # FINE-TUNE: resume the FULL model (UNet + transformer) from a saved checkpoint, then continue training.
    _resume = os.environ.get("BIOHUB_RESUME_CKPT", "").strip()
    if _resume:
        _rp = Path(_resume)
        assert _rp.exists(), f"BIOHUB_RESUME_CKPT not found: {_rp}"
        _state = torch.load(_rp, map_location=device, weights_only=True)
        _missing, _unexpected = model.load_state_dict(_state, strict=False)
        print(f"  FINE-TUNE resume from {_rp}: {len(_missing)} missing, {len(_unexpected)} unexpected keys", flush=True)
