...
# --- Instrumentation (fork addition): dump candidate probs + node appearance embeddings -----------------
# Default OFF → this fork reproduces the stock predict exactly (geff unchanged). When BIOHUB_DUMP_SIDECARS=1,
# writes {name}.candidates.parquet (all src×tgt pairs with prob > DUMP_THRESHOLD, keyed by geff node_id) and
# {name}.embeddings.npz (per-node 32-ch UNet appearance vector). DUMP_THRESHOLD is well below cfg.threshold so
# the correct-but-under-scored far edge is captured; the real candidate set / geff still uses cfg.threshold.
DUMP_SIDECARS = os.environ.get("BIOHUB_DUMP_SIDECARS", "0") != "0"
...
def _write_sidecars(output_dir: Path, name: str, dump_edges: list, node_emb: dict) -> None:
    ...
    if node_emb:
        ...
        np.savez_compressed(output_dir / f"{name}.embeddings.npz", node_ids=ids, embeddings=emb)
...
def predict_video(
    ...
    for ws in tqdm(
        ...
        for f_idx in range(W - 1):
            ...
            if DUMP_SIDECARS:
                _es = unet_feat_src.detach().cpu().numpy().reshape(len(idx_src), -1)
                _et = unet_feat_tgt.detach().cpu().numpy().reshape(len(idx_tgt), -1)
                for _k, _g in enumerate(idx_src):
                    node_emb.setdefault(int(_g), _es[_k])
                for _k, _g in enumerate(idx_tgt):
                    node_emb.setdefault(int(_g), _et[_k])
